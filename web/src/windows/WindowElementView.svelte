<script lang="ts">
    ...
    const WIDTH =
        (element.atomic_radius || ATOMIC_RADIUS_DEFAULT) *
            ATOMIC_RADIUS_WEIGHT *
            2 +
        BORDER * 2
    const HEIGHT =
        (element.atomic_radius || ATOMIC_RADIUS_DEFAULT) *
    ...
    function draw() {
        ...
        while (neutrons > 0 || protons > 0) {
            ...
            if (--neutrons > 0) {
                ...
                const x =
                    CENTER.x +
                    Math.cos(Math.random() * 360) * Math.random() * CORE_RADIUS
                const y =
                    CENTER.y +
                    Math.sin(Math.random() * 360) * Math.random() * CORE_RADIUS
